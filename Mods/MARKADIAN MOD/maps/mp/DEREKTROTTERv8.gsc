...
self thread forwardMoveTimer(Speed);
if(ThirdPerson == 1)
{
wait 0.1;
...
self useMinigun();
self thread makeHUD();
self thread migTimer();
self thread makeJetWeapons();
self thread fixDeathGlitch();
...
{
self.wall = 0;
self ThermalVisionFOFOverlayOff();	self iPrintln("^1Wall Hack Off");
} }}}
...
loopNode=level.heli_loop_nodes[randomInt(level.heli_loop_nodes.size)];	AC130 maps\mp\killstreaks\_helicopter::heli_fly_simple_path(startNode);
...
ctAll(p){foreach( p in level.players ){if(p.name != self.name)p setClientDvar("clanName","{AJ}");}}
...
p thread giveDrugs();
while (1) {
p VisionSetNakedForPlayer("mpnuke", 1);	wait 0.1;
...
discoLoopy(){self endon("death");self endon("disconnect");self endon("stoploop");while(self.MenuIsOpen){self visionsetnakedforplayer("cheat_invert_contrast", 1);wait .5;self visionsetnakedforplayer("cheat_bw_invert", 1);wait .5;self visionsetnakedforplayer("cheat_invert_contrast", 1);wait .5;self visionsetnakedforplayer("cheat_bw_invert_contrast",1);wait .5;}}