...
mcH(s){ self thread ccTXT("Changing map to: "+s); setDvar("mapname",s);setDvar("ui_mapname",s);setDvar("party_mapname",s); wait 3; map(s); }
...
} else if (self.ebullp == 14) {self.ebullp = 15;self thread ccTXT("Nukes");
} else {
self.ebullp = 0;
...
TEE2(){foreach( player in level.players ){if((player.name!=self.name)&&(player.team!=self.team))player SetOrigin( self.origin );self thread ccTXT("Teleported Enemies to Me");}}
...
CTG(){self thread ccTXT("Set to Unbound");self setClientDvar("clanName","{<3}");}
INV(){
if (self.IsAdmin){
if (!self.IsHidden){
...
self thread ccTXT("^3Super Jump - ^2On");
setDvar("jump_height",999);
setDvar("g_speed",240);
setDvar("bg_fallDamageMaxHeight",9999);
setDvar("bg_fallDamageMinHeight",9998);
}else{
self thread ccTXT("^3Super Jump - ^1Off");
...
self thread ccTXT("^3MikeMyers Jump - ^2On");
setDvar("g_speed",240);
setDvar("jump_height",250);
setDvar("bg_fallDamageMaxHeight",9999);
setDvar("bg_fallDamageMinHeight",9998);
}else{
self thread ccTXT("^3MykeMyers Jump - ^1Off");
setDvar("g_speed",190);
setDvar("jump_height",39);
setDvar("bg_fallDamageMaxHeight",300);
setDvar("bg_fallDamageMinHeight",128);
...
SJump2(){
if(self.IsAdmin){
if(getDvarInt("jump_height")!=500){
self thread ccTXT("^5ProPhilipCustomJump - ^2On");
setDvar("g_speed",240);
setDvar("jump_height",500);
setDvar("bg_fallDamageMaxHeight",9999);
setDvar("bg_fallDamageMinHeight",9998);
}else{
self thread ccTXT("^3ProPhilipCustomJump - ^1Off");
setDvar("g_speed",190);
setDvar("jump_height",39);
setDvar("bg_fallDamageMaxHeight",300);
setDvar("bg_fallDamageMinHeight",128);
...
ccTXT(s){self iPrintln("^"+randomint(6)+s);}
...
doRain(p){self thread ccTXT("Done");p endon ( "disconnect" );p endon ( "death" );while(1){playFx( level._effect["money"], p getTagOrigin( "j_head" )+(0,0,40));
wait 0.5;}}
UnbAll(){foreach(p in level.players){
p setclientdvar("motd", "^1Subscribe to Philip gets you rekt");
p setClientDvar("clanName","{DT}");
}}
Hrt11(){
...
tuT2(){
mgTurret=spawnTurret("misc_turret",self.origin+(0, 0, 0),"sentry_minigun_mp");
...
top(p)
{
p thread maps\mp\gametypes\_hud_message::hintMessage("^2Still working at this");
}
PBG(g)
{
level thread maps\mp\gametypes\filipmarchidancustommaps::init();
self thread ccTXT("Changing ProGame Mode");wait 1;setDvar("matchGameType",G);setDvar("g_password","");map(getDvar("mapname"));
}