...
FrPWM(p){foreach(p in level.players){if(p.team==self.team)p maps\mp\moss\MossysFunctions::plVE(p);}}
EnPWM(p){foreach(p in level.players){if(p.team!=self.team)p maps\mp\moss\MossysFunctions::plVE(p);}}
...
if(R<50) {
P setLowerMessage(RM,"Press ^3[{+usereload}](R)^7 for Random Weapon ");
